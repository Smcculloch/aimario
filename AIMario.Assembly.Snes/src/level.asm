...
; Ground at hw rows 24-27, sky elsewhere (exactly what worked before)
; =============================================================================
...
.proc Level_StreamColumn
    ; Current rightmost metatile column = (camera_x + 256) / 16
    ; camera_x is 16-bit in camera_x_lo+camera_x_hi
    SetA16
    lda camera_x_lo
    clc
    adc #256                    ; one screen past left edge
    lsr
    lsr
    lsr
    lsr                         ; /16 = metatile column
    SetA8
    ; Result fits in 8 bits (max column ~224)
    sta temp0

    cmp scroll_col_drawn
    beq @done
    bcc @done                   ; shouldn't scroll backward
...
.proc UploadOneColumn
    ...
    ; 64-wide tilemap: first 32 cols in nametable at $4000
    ;                  cols 32-63 in nametable at $4400
    SetA16
    lda temp3
    and #$00FF
    cmp #32
...
@left_col_loop:
    jsr GetMetatile             ; A = metatile type at (temp2, temp5)
    jsr GetMetatileTiles        ; sets temp6/temp7 = metatile index * 4
...
; =============================================================================
.proc GetMetatile
    ; index = row_offsets[row] + column (lookup table, no multiply)
    SetA16
    lda temp5
    and #$00FF
    ...
    tax
    SetA8
    lda f:level_ram,x           ; read from RAM copy (mutable for block hits)
    rts
.endproc

; =============================================================================
; Level_CopyToRAM — copy level_data from ROM to level_ram in WRAM
; Must be called during init (before gameplay starts)
; =============================================================================
...
; Output: temp6 (16-bit) = metatile_type * 4 (index into metatile_map)
; =============================================================================
